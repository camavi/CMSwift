  ...
  const renderDocGroup = (title, items, opts = {}) => {
    ...
    return _.div({ class: uiClass(["cms-component-docs-surface", opts.className]) },
  ...
  UI.ComponentDocs = (...args) => {
    const { props, children } = CMSwift.uiNormalizeArgs(args);
    const doc = isUIPlainObject(props.doc) ? props.doc : {};
    const rawName = props.name || doc.name || doc.component || "";
    const meta = UI.meta?.[rawName] || null;
    ...
    const signature = signatureRaw ? String(signatureRaw).replaceAll("UI.", "_.") : "";
    ...
    rootProps.class = uiClass(["cms-component-docs", `is-${status}`, props.class]);
  ...
  if (CMSwift.isDev?.()) {
    UI.meta = UI.meta || {};
    UI.meta.ComponentDocs = {
      signature: "UI.ComponentDocs({ doc, api? })",
      props: {
        doc: "object",
        name: "string",
        title: "string",
